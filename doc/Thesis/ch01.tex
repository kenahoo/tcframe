...
object-oriented software development.  Frameworks assist developers in
creating multiple similar, but non-identical, applications.

The literature on frameworks contains several different definitions of
the term:
...
\item XXX - definition from \cite{gamma:95}
\end{itemize}
...
what a framework is made of, the second emphasizes what a framework is
used for, and the third emphasizes XXX.

Several terms are commonly encountered in the literature on
frameworks.  

requiring fine-grained control over the \ldots
XXX \ldots
...
called \emph{hot spots}, and allow explicitly for their variations to
be instantiated in applications.  In some cases, the hot spot
...
for the application.  This is known as \emph{black box} framework
usage.  In other cases, the application developer may have a
particular need that the framework developer did not or could not
anticipate.  Application development then involves writing custom
subclasses of the hot spot classes, a process known as \emph{white
box} framework usage.

Because black box framework usage involves much less effort than white
box usage, ...
