...
interface decisions will be discussed in detail.  Implementation will be
...
have a graphical interface for running common kinds of experiments.
...
for creating these kinds of applications.  Since a single
...
A fellow student at the University of Sydney, David Bell, has created
an interface between the PostgreSQL relational database and the
\aicat\ framework, allowing the retrieval of documents from the
database and access to categorization functionality via SQL functions
\cite{williams:03}.  Although the SQL interface functions will need to
be rewritten for each database system (e.g. Oracle, Sybase, MySQL),
the core framework functionality can be reused.
