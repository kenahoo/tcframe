...
most important ones for this discussion.  For a complete reference,
please see the \aicat\ documentation.
...
\begin{concreteclass}{KnowledgeSet}
 \attributes
  ...
  \class{FeatureSelector} feature\_selector
 \methods
  documents() \\
  categories() \\
  scan\_features(\class{Collection})
\end{concreteclass}
...
\begin{abstractclass}{FeatureSelector}
 \attributes
  \class{number} features\_kept
 \methods
  reduce\_features(\class{FeatureVector}) \\
  rank\_features() \\
  scan\_features() \\
  select\_features()
\end{abstractclass}
