...
XXX - clarify relationship between word, token, and feature
...
In addition, the tokenization process 
may remove from the data any term in a
...
gain} (IG) metrics. \cite{yang:97} $\text{DF}(f_k)$ is simply the
number of documents of $\train$ in which the feature $f_k$ occurs.
$\chi^2$ is defined in Equation \ref{eqn-x2-def}. $A$, $B$, $C$, and $D$ are
defined as the terms in the contingency table shown in Table
\ref{termcat-contingency}.

XXX - explain what a chi^2 score of 1 means, and what 0 means, about
...
XXX - indicate whether IG has been found to be good, and what good/bad
IG scores might be.

\subsection{Vector space modeling}

XXX - move this section to before dim-reduction.
...
These three techniques were chosen for inclusion here because they
currently have implementations in the \aicat\ framework and because
they are included in the well-known study \cite{yang:99}.  Their
...
XXX mention scrutability.

Because of time considerations, a need to limit the scope in which
testing was performed, and the fact that \cite{yang:99} does not
