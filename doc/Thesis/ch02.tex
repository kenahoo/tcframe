...
The other two metrics are defined as follows---here, $A$, $B$, $C$,
and $D$ are defined as the terms in the contingency table shown in
Table \ref{termcat-contingency}, and $\Entr(x)$ and $\Entr(x|y)$ are the
standard entropy and conditional entropy functions from Information
Theory.\cite[ch. XXX]{manning:99}
...
$\chi^2(f_k,c_j)$ may either be averaged, or the maximum value for any
category may be adopted.
...
Homeland Security.  Its products range from 
