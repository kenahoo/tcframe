...
and released under an open source license\cite{XXX-open-source} as a
...
\item Perl is widely known to be a powerful text processing tool
   \cite{XXX-mastering-regular-expressions, etc}, hence it should be
...
\item Perl can be embedded within applications written in other
   ...
   Java/Perl Layer.  This allows for maximum reusability of the
...
To implement the behavior discussed in Section \ref{factory-method},
the \class{Class::Container} module from CPAN\footnote{The
\class{Class::Container} module was written by Ken Williams for a
previous project\cite{rolsky:02} and extended for the \aicat\
...
\method{new} method in an aggregation\cite[XXX-section]{gamma:95}
manner.
...
in the previous paragraph is currently underway by another researcher
at the University of Sydney, though it is not yet a part of the
\aicat\ framework.

\subsection{Sets of Documents or Categories}

The \class{KnowledgeSet} class needs to keep track of 
